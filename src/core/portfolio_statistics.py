...
class PortfolioStatistics:
    ...
    def calculate_sharpe_ratio(self, lookback_trades: int = 30) -> Dict:
        """计算夏普比率

        Args:
            lookback_trades: 回溯的交易数量（默认30笔）
        ...
        Returns:
            {
                ...
                'confidence_threshold': int,  # 信心度门槛
        ...
        # 提取收益率（pnl_percent）
        returns = [trade['pnl_percent'] for trade in recent_trades]
        ...
        if std_dev == 0 or std_dev < 1e-10:
            # 所有交易收益率完全相同（极其罕见）
            if avg_return > 0:
                sharpe_ratio = 999.99  # 表示"完全确定的盈利"
                print(f"💎 标准差为0且盈利，夏普比率设为极高值")
            elif avg_return < 0:
                sharpe_ratio = -999.99  # 表示"完全确定的亏损"
                print(f"⚠️ 标准差为0且亏损，夏普比率设为极低值")
            else:
                sharpe_ratio = 0.00  # 无收益无波动
        else:
            # 年化调整：假设每天20笔交易（5分钟间隔，24小时运行）
            # 年化因子 = sqrt(交易频率 * 365)
            # 简化版：直接使用原始数据，不做年化
            sharpe_ratio = avg_return / std_dev
            # 限制在合理范围内，防止溢出
            sharpe_ratio = max(-999.99, min(999.99, sharpe_ratio))
    ...
    def generate_stats_text_for_ai(self) -> str:
        ...
        # 计算夏普比率
        sharpe_info = self.calculate_sharpe_ratio()
    ...
    【风险管理状态】⚠️ 重要
    - 夏普比率: {sharpe_info['sharpe_ratio']:.2f} （基于最近{sharpe_info['sample_size']}笔交易，{sharpe_info['note']}）
    - 当前风险等级: {sharpe_info['risk_level']}
    - 最大持仓限制: {sharpe_info['max_positions']}个
    - 开仓信心度门槛: ≥{sharpe_info['confidence_threshold']}
