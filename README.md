...
| 特性 | 普通机器人 | 本项目 |
|------|----------|--------|
| **时段分析** | 单一周期（如15分钟） | **15分钟+1小时+4小时** 三周期交叉验证 ✨ |
| **大盘参考** | 只看单币数据 | **每次都参考BTC大盘情绪** 🎯 |
| **决策依据** | 单一维度判断 | **短中长期结合**，避免被假突破误导 🛡️ |
| **可靠性** | 容易被短期波动欺骗 | **多时段互相验证**，更稳健 💪 |

**为什么多时段分析重要？**
- 📉 15分钟看到上涨 → 1小时发现下跌趋势 → AI会谨慎观望
- 📈 15分钟+1小时+4小时都看涨 → BTC也在上涨 → AI高信心开多
- 🎯 大幅降低假信号，提高胜率
...
### 📈 技术分析（核心优势）
- **🌟 多时段交叉验证**：15分钟 + 1小时 + 4小时 三周期分析
- **🔥 短中长期结合**：15分钟捕捉短期机会，1小时把握趋势，4小时验证方向
...
- `check_interval_minutes`: AI分析间隔（**不建议修改**）
  - 默认：5分钟
  - ⚠️ **修改会导致K线数据不匹配**：程序使用 **15分钟 + 1小时 + 4小时** 三个时段的K线数据进行多时段交叉验证，5分钟间隔可以完整捕获15分钟K线的变化
  - 如果改为其他值（如10分钟），会错过关键的K线形态变化
- `test_mode`: 测试模式开关
  - `False`: 实盘模式，真实下单
