...
| 特性 | 普通机器人 | 本项目 |
|------|----------|--------|
| **时段分析** | 单一周期（如15分钟） | **多周期交叉验证**：5m K线形态 + 15m/30m/2h技术分析 + BTC 15m/1h/4h ✨ |
| **大盘参考** | 只看单币数据 | **每次都参考BTC大盘情绪** 🎯 |
| **决策依据** | 单一维度判断 | **短中长期结合**，避免被假突破误导 🛡️ |
| **可靠性** | 容易被短期波动欺骗 | **多时段互相验证**，更稳健 💪 |

**为什么多时段分析重要？**
- 📉 5分钟K线形态看涨 → 但30分钟/2小时发现下跌趋势 → AI会谨慎观望
- 📈 5m/15m/30m/2h都看涨 → BTC的15m/1h/4h也看涨 → AI高信心开多
- 🎯 大幅降低假信号，提高胜率
...
### 📈 技术分析（核心优势）
- **🌟 多时段交叉验证**：
  - 币种K线形态：5分钟K线实时监控（最近24根，共2小时）
  - 币种技术分析：15分钟（主要） + 30分钟 + 2小时 三周期
...
- `check_interval_minutes`: AI分析间隔（**不建议修改**）
  - 默认：5分钟
  - ⚠️ **修改会导致K线数据不匹配**：程序使用 **5分钟K线形态** + **15/30分钟/2小时技术分析**（币种）和 **15分钟/1小时/4小时**（BTC）进行多时段交叉验证，5分钟间隔正好匹配5分钟K线周期
  - 如果改为其他值（如10分钟），会错过5分钟K线的关键形态变化
- `test_mode`: 测试模式开关
  - `False`: 实盘模式，真实下单
