...
> **💭 作者的话**  
> *"经历了无数次追涨杀跌、情绪化操作后，我对自己的交易水平彻底失望了。*  
> *与其被恐惧和贪婪支配，不如把决策交给AI——至少它不会因为行情波动而失眠。*  
> *这个项目源于一个简单的信念：AI再不完美，也比情绪失控的我更理性。"*
> 
> **⚠️ 重要提醒**  
...
> **🤔 为什么需要最小余额？这和 `min_order_qty` 有什么区别？**
> 
> 这是两个不同的限制：
> - **`min_order_qty`（最小订单数量）**：币安交易所的硬性规定，单笔订单最少要多少个币（如 0.01 BNB）
> - **`balance > 10`（最小账户余额）**：我们的风险控制，账户至少要有多少 USDT 才允许开仓
> 
> **为什么需要余额限制？**
> 
> | 场景 | 账户余额 | BNB价格 | 无余额限制 | 有余额限制(>10) |
> |------|---------|---------|-----------|----------------|
> | **极小账户** | 5 USDT | 1100 | 尝试开仓 → 💥 下单失败<br>（0.01 BNB = 11 USDT > 5 USDT） | ❌ 拒绝开仓<br>✅ 避免失败 |
> | **刚好够一单** | 12 USDT | 1100 | 开仓 11 USDT → ⚠️ 剩1U<br>波动10%就强平 | ❌ 拒绝开仓<br>✅ 保护资金 |
> | **足够余额** | 50 USDT | 1100 | ✅ 正常开仓 | ✅ 正常开仓 |
> 
> **总结**：账户余额限制是为了防止用极小账户开仓，避免下单失败或轻易被强平！
...
- 设置太低可能导致：
  ...
  - 剩余保证金太少 → 轻易触发强平
- **建议根据交易币种调整**：
  - BNB/SOL (单价~1000): 建议 ≥ 20 USDT
  - ETH (单价~3000): 建议 ≥ 50 USDT
  - BTC (单价~60000): 建议 ≥ 100 USDT
