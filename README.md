...
### 止盈机制
- **记录为目标价**：AI 提供的 `take_profit` 当前作为目标价记录与展示，便于 AI 在后续周期自行决定是否平仓。
- **未挂交易所 TP 单**：目前未实现“取消旧 TP + 挂新 TP”的移动止盈流程；如需与止损同样的自动挂单机制，可后续扩展为 `TAKE_PROFIT_MARKET` 并在代码中增加撤旧+重挂逻辑。

### AI 调用超时保护
- **超时时间**：120 秒
- **重试机制**：OpenAI SDK 默认重试（503/429 错误自动重试 2 次，指数退避）
- **超时处理**：如果 AI 服务 120 秒内未返回，自动跳过本次决策（HOLD）
- **程序保护**：避免因 AI 服务异常导致程序卡死，确保下一周期正常执行

**工作流程**：
1. AI 正常返回（< 120秒）→ 执行决策
2. AI 超时（≥ 120秒）→ 返回空决策，记录日志，等待下一周期
3. API 错误（503/429）→ 自动重试，累计时间计入 120 秒超时
