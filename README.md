...
与其每天假装看盘实则在赌，不如把决策交给 AI——至少它不会因为行情波动而梭哈。
这个项目源于一个简单的期望：AI 再不完美也比我强。”
...
您的支持是持续更新的动力 ❤️ 没有收费也没有挂邀请码，只求各位给星星 ⭐

# 🚀 AI多币种投资组合管理系统

⚠️ 仅支持单向持仓（每个币种同一时刻只持有一个方向的一笔仓位；如需反向，先平后开；不支持同标的双向/多子仓并存）。
一套基于大模型的多币种合约交易与风险管理程序，策略可外置、止损强制落地、支持移动止损。

## 🔧 提示词外置化（超重大更新）
本项目将“交易策略与行为规范”从代码中彻底剥离：代码只负责收集数据、调用 AI、下单与风控；“怎么分析、如何做决策”交给外部提示词文件管理。这样改有什么用：
- 策略改动无需动代码，直接改文案即可上线；
- 支持多套策略版本并随时切换（激进/保守/均衡等）；
- 降低回归风险，代码层只保留最小“红线”约束。

放在哪里：`prompts/default.txt`（可复制为多个版本并手动切换加载路径）。

提示词分层说明
- 外置提示词（你可编辑）
  - 交易身份定位、风格与目标
  - 多周期分析框架（5m/15m/1h/4h 的角色与用法）
  - 入场/加仓/平仓的信号标准与优先级
  - 仓位管理建议（强/中/弱信号的仓位建议）
  - 止损/止盈策略（如 ATR 参考、追踪止盈思路）
  - 历史回顾使用方式（胜率/盈亏比/连亏抑制/高胜率强化）

- 硬提示词（System Message，代码内置，不建议修改）
  - 返回格式：必须输出规范 JSON（decisions/strategy/risk_level/confidence）
  - 资金红线：最大可用保证金 = 账户总资产 × 90% − 已用保证金
  - 最小开仓金额：全局硬下限 13 USDT；同时加载各币种最小额（取较大值）
  - 杠杆：由配置管理（默认 5x），AI 不直接调杠杆
  - 止损必填：开仓与持仓（HOLD）都必须提供 stop_loss
  - 移动止损机制：HOLD 给新 stop_loss → 系统撤旧+重挂止损单

数据作为 User Message 提供给 AI
- 账户资金与风险指标（总资产、已用保证金、可用余额等）
- BTC 大盘与各币种多周期 K 线和技术指标
- 当前持仓信息（方向、数量、浮盈浮亏、止损/止盈）
- 历史统计与最近决策、止损触发记录

小结：
- 你可以随时编辑 `prompts/default.txt` 来改变“如何交易”；
- 系统只把“必须安全/必须可解析”的规则硬编码，避免 AI 越线或输出不可解析结果。

DeepSeek AI管理7币种投资组合：BTC、ETH、SOL、BNB、XRP、ADA、DOGE
